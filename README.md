...
You may also wish to change the base namespace from `Ran\StarterPlugin` to something else. This can be typically achieved by a search and replace (yolo). One done, you will need to update the `composer.json` file to reflect the new namespace, and then run `composer dump-autoload` to update the autoloader.
...
`pnpm run stylelint` uses stylelint to lint scss
`pnpm run eslint` uses eslint to lint JavaScript
`pnpm run lint` lints both styles and JavaScript
`pnpm run format` formats your code with Prettier

#### Linting PHP

The project has included [WordPress Coding Standards](https://github.com/WordPress/WordPress-Coding-Standards) with [PHP Code Sniffer](https://github.com/PHPCSStandards/PHP_CodeSniffer/) as `dev` dependencies using Composer.
...
PHP_CodeSniffer version 3.7.1 (stable) by [Squiz](https://www.squiz.net)
...
The project uses [Vite](https://vitejs.dev/) to compile Sass and JavaScript which allows for Hot Module Reloading to speed up your development, and a host of other features.
...
> NOTE: HMR fundamentally changes the contents of scripts and styles. Be sure to run your `build` command prior to committing resources to production!

### Vite Configuration

The project uses a single `vite.config.js` file that handles both admin and public assets. The configuration maintains the same directory structure as before:
...
export default defineConfig({
  build: {
    ...
    rollupOptions: {
      ...
      // Output configuration maintains the same directory structure
    },
  },
});
```
...
The project uses [Browserslist](https://github.com/browserslist/browserslist) to configure browser compatibility. The configuration extends the WordPress browserslist configuration:
...
Vite handles Sass compilation natively with the sass package. You can import from node_modules directly without any additional configuration:

```scss
// Example of importing from node_modules
...
The pattern we have implemented is as follows:
